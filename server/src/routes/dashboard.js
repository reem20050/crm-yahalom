    ...
    if (req.user.role === 'employee' && req.user.employeeId) {
      ...
      const [myShiftsToday, myUpcomingEvents, myRecentShifts, myEquipment] = await Promise.all([
        ...
        db.query(`
          ...
          WHERE sa.employee_id = $1 AND s.date = CURRENT_DATE
    ...
    const [
      ...
      db.query(`
        SELECT
          ...
          SUM(CASE WHEN status = 'won' AND updated_at >= DATE_TRUNC('month', CURRENT_DATE)::date THEN 1 ELSE 0 END) as won_this_month
        FROM leads
      `),
      ...
      db.query(`
        SELECT
          ...
          SUM(CASE WHEN created_at >= DATE_TRUNC('month', CURRENT_DATE)::date THEN 1 ELSE 0 END) as new_this_month
        FROM customers
      `),
      ...
      db.query(`
        ...
        FROM invoices
        WHERE issue_date >= DATE_TRUNC('month', CURRENT_DATE)::date - INTERVAL '5 months'
  ...
  try {
    ...
    const [
      ...
      safeQuery(`
        ...
        FROM shift_assignments sa
        JOIN shifts s ON sa.shift_id = s.id
        WHERE s.date = CURRENT_DATE AND sa.status = 'checked_in'
      `),
      ...
      safeQuery(`
        ...
        WHERE s.date = CURRENT_DATE AND s.site_id IS NOT NULL AND sa.status = 'checked_in'
      `),
      // Open incidents
      safeQuery(`
      ...
      safeQuery(`
        ...
        WHERE s.date = CURRENT_DATE
          ...
          AND s.start_time <= TO_CHAR(NOW() + INTERVAL '30 minutes', 'HH24:MI')
        ORDER BY s.start_time
      `),
  ...
  try {
    ...
    const [monthlyRevenue, prevMonthRevenue, activeCustomers, prevActiveCustomers,
      ...
      safeQuery(`SELECT COALESCE(SUM(total_amount), 0) as total FROM invoices
        WHERE status = 'paid' AND payment_date >= DATE_TRUNC('month', CURRENT_DATE)::date`),
      // Previous month revenue
      safeQuery(`SELECT COALESCE(SUM(total_amount), 0) as total FROM invoices
        WHERE status = 'paid' AND payment_date >= DATE_TRUNC('month', CURRENT_DATE)::date - INTERVAL '1 month'
      ...
      safeQuery(`SELECT COUNT(*) as count FROM incidents
        WHERE created_at >= DATE_TRUNC('month', CURRENT_DATE)::date - INTERVAL '1 month'
        AND created_at < DATE_TRUNC('month', CURRENT_DATE)::date`),
      // Employee utilization (shifts assigned / total employees)
      safeQuery(`SELECT
        (SELECT COUNT(DISTINCT sa.employee_id) FROM shift_assignments sa
         JOIN shifts s ON sa.shift_id = s.id
         WHERE s.date BETWEEN CURRENT_DATE - INTERVAL '7 days' AND CURRENT_DATE) as active_employees,
    ...
    let targets = {};
    try {
      const targetRows = await db.query(`SELECT metric, target_value FROM kpi_targets
...
router.post('/kpi-targets', requireManager, async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM kpi_targets WHERE metric = $1 AND period = $2', [metric, effectivePeriod]);
    await db.query(`INSERT INTO kpi_targets (id, metric, target_value, period, start_date, end_date, created_by)
      VALUES ($1, $2, $3, $4, DATE_TRUNC('month', CURRENT_DATE)::date, (DATE_TRUNC('month', CURRENT_DATE)::date + INTERVAL '1 month' - INTERVAL '1 day')::date, $5)`,
