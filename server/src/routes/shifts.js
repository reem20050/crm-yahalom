...
router.post('/bulk-approve', requireManager, async (req, res) => {
  try {
    ...
    await db.query(`UPDATE shifts SET status = 'confirmed' WHERE id IN (${placeholders}) AND status = 'draft'`, shift_ids);
...
router.post('/bulk-delete', requireManager, async (req, res) => {
  try {
    ...
    const placeholders = shift_ids.map(() => '?').join(',');
    await db.query(`DELETE FROM shift_assignments WHERE shift_id IN (${placeholders})`, shift_ids);
    await db.query(`DELETE FROM shifts WHERE id IN (${placeholders})`, shift_ids);
...
router.post('/copy-week', requireManager, async (req, res) => {
  try {
    ...
    const sourceShifts = await db.query(`
      SELECT s.*,
...
router.post('/generate-from-template/:templateId', requireManager, async (req, res) => {
  try {
    ...
    const template = await db.query('SELECT * FROM shift_templates WHERE id = ?', [templateId]);
...
router.post('/swap-requests/:requestId/resolve', requireManager, async (req, res) => {
  try {
    ...
    const request = await db.query('SELECT * FROM shift_swap_requests WHERE id = ?', [requestId]);
