...
const rateValidation = [
  body('employee_id').notEmpty().withMessage('עובד חובה'),
  body('rating').isInt({ min: 1, max: 5 }).withMessage('דירוג חייב להיות בין 1 ל-5'),
];
...
router.post('/rate', rateValidation, async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT * FROM guard_ratings WHERE id = ?', [id]);
