...
router.post('/', requireManager, async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT * FROM guard_certifications WHERE id = ?', [id]);
...
router.put('/:id', requireManager, async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT * FROM guard_certifications WHERE id = ?', [req.params.id]);
...
router.delete('/:id', requireAdmin, async (req, res) => {
  try {
    await db.query('DELETE FROM guard_certifications WHERE id = ?', [req.params.id]);
