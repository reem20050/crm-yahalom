...
class WhatsAppService {
  ...
  // Load WAHA URL + API key from DB if set (mutex to prevent race condition)
  async _ensureConfig() {
    // If a config load is already in progress, reuse it
  ...
  async _loadConfig() {
    try {
      ...
      console.error('[WhatsApp] _loadConfig failed:', e.message);
    }
    return false;
  }
