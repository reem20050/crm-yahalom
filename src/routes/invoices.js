...
router.post('/bulk-approve', requireManager, async (req, res) => {
  try {
    ...
    await db.query(`UPDATE invoices SET status = 'sent' WHERE id IN (${placeholders}) AND status = 'draft'`, invoice_ids);
...
router.post('/bulk-delete', requireManager, async (req, res) => {
  try {
    ...
    const placeholders = invoice_ids.map(() => '?').join(',');
    await db.query(`DELETE FROM invoices WHERE id IN (${placeholders})`, invoice_ids);
