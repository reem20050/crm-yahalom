...
router.post('/bulk-status', requireManager, async (req, res) => {
  try {
    ...
    await db.query(`UPDATE leads SET status = ?, updated_at = datetime('now') WHERE id IN (${placeholders})`, [status, ...lead_ids]);
...
router.post('/', requireManager, [
  ...
  try {
    ...
    whatsappHelper.notifyNewLead(result.rows[0]).catch(err => {
...
router.put('/:id', requireManager, async (req, res) => {
  try {
    ...
    if (status) {
      ...
      whatsappHelper.notifyLeadStatusChange(result.rows[0], status, assignedUser?.phone).catch(err => {
        console.error('WhatsApp notification failed:', err.message);
      });
    }
